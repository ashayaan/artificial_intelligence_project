...
int main(int argc, char **argv)
{
	...
	if(mode & MODE_VALIDATE)
	{
		...
		if(toolboxType == TOOLBOX_HCRF || toolboxType == TOOLBOX_GHCRF )
			dataTrain.load((char*)filenameDataTrain.c_str(),NULL, (char*)filenameLabelsTrain.c_str());
		else
	...
	if(mode & MODE_TRAIN)
	{
		...
		if(toolboxType == TOOLBOX_HCRF || toolboxType == TOOLBOX_GHCRF )
			data.load(fileData,NULL, (char*)filenameLabelsTrain.c_str(),NULL,NULL,fileDataSparse);
		else
	...
	if(mode & MODE_TEST)
	{
		...
		DataSet data;
		if(toolboxType == TOOLBOX_HCRF || toolboxType == TOOLBOX_GHCRF )
			data.load((char*)filenameDataTest.c_str(),NULL,(char*)filenameLabelsTest.c_str());
		else
